...
const getCheckoutInfo = async() => {
  ...
  //这里item.isDefault===0,才是默认地址，是因为后端接口问题没有0这个地址
  // 数组中find默认查找的为第一项
  ...
  <!-- 切换地址 -->
   <el-dialog v-model="showDialog" title="切换收货地址" width="30%" center>
    <div class="addressWrapper">
      <div :class="{active:activeAddress.id === item.id}" class="text item" @click="switchAddress(item)" v-for="item in checkInfo.userAddresses"  :key="item.id">
        <ul>
...
.text {
  ...
  &.item {
    ...
    &:hover {
      border-color: $xtxColor;
      background: color.scale($xtxColor, $lightness: 89%);
    }
